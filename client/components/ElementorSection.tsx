...
import { fadeUp, slideInLeft, slideInRight, inViewConfig } from "@/lib/animations";
...
const features = ["Experienced Crew", "Cinematic Quality", "Authentic Storytelling", "Multi-Platform Delivery"];
...
export default function ElementorSection() {
  ...
  return (
    <section className="w-full bg-black py-20 px-4" ref={ref} aria-labelledby="production-heading">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12">
        ...
        >
          ...
          <p className="text-[#D1D0CF] font-['Roboto'] text-base md:text-lg max-w-lg leading-relaxed">
            Behind every Araceli Entertainment production is a team of industry
            professionals committed to delivering blockbuster quality content
            that tells meaningful stories.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 mt-2">
            {features.map((feat, i) => (
              <motion.div
                key={feat}
                className="flex items-center gap-3 text-white font-['Roboto'] text-base capitalize"
                variants={fadeUp}
                initial="hidden"
                animate={inView ? "visible" : "hidden"}
                custom={0.15 + i * 0.1}
